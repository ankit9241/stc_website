...
import { Loader2, Trophy, Medal, Award, Search, Filter, Mail, Building2 } from "lucide-react"
...
export default function ResultsPage() {
  ...
  const filterResults = () => {
    ...
    if (selectedCompetition !== 'all') {
      filtered = filtered.filter(result => result.competitionName === selectedCompetition)
    }
  ...
  const getRankBadge = (rank: number) => {
    const badges = {
      1: { label: '1st Place', color: 'bg-yellow-500 hover:bg-yellow-600 text-white' },
      2: { label: '2nd Place', color: 'bg-gray-400 hover:bg-gray-500 text-white' },
      3: { label: '3rd Place', color: 'bg-amber-600 hover:bg-amber-700 text-white' }
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 pt-24 pb-12 px-3 sm:px-4 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 sm:mb-8 space-y-3 max-w-5xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            {(searchQuery || selectedClub !== 'all' || selectedCompetition !== 'all') && (
              <Button
                variant="outline"
                onClick={() => {
                  setSearchQuery('')
        ...
        {filteredResults.length > 0 && (
          <div className="text-center mb-6 sm:mb-8">
            <p className="text-sm sm:text-base text-gray-600">
              Showing <span className="font-semibold text-[#1a4b8c]">{filteredResults.length}</span> {filteredResults.length === 1 ? 'result' : 'results'}
              {Object.keys(groupedResults).length > 0 && (
                <> across <span className="font-semibold text-[#1a4b8c]">{Object.keys(groupedResults).length}</span> {Object.keys(groupedResults).length === 1 ? 'competition' : 'competitions'}</>
              )}
            </p>
          </div>
        )}
          ...
          <div className="space-y-8 sm:space-y-10">
            {Object.entries(groupedResults)
              ...
              .map(([competitionName, competitionResults]) => {
                ...
                const sortedResults = [...competitionResults].sort((a, b) => a.rank - b.rank)
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex flex-wrap items-center gap-3 text-xs sm:text-sm text-blue-100">
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              <span>{sortedResults.length} {sortedResults.length === 1 ? 'Winner' : 'Winners'}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0 space-y-2">
                                <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                                  ...
                                  <Badge className={`${badge.color} text-xs sm:text-sm w-fit`}>
                                    {badge.label}
                                  </Badge>
                                </div>
                                ...
                                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs sm:text-sm text-gray-600">
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="truncate">{result.collegeMail}</span>
                                  </div>
                                  <div className="flex items-center gap-1.5">
                                    <span className="font-medium text-[#1a4b8c]">Roll No:</span>
